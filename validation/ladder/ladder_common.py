...
def run_cli(engine_name: str, pct, CreditPolicy, col, l8_notes: dict) -> None:
    ...
    ladder step, and writes ONE consolidated JSONL per (engine, tag) — one record
    per line. The whole ``results/`` tree is gitignored (regenerable local data);
    only ``ladder_summary.md`` (from build_summary.py) is committed."""
    import argparse
    import json
    ...
    for step in STEPS:
        ...
        print(f"{engine_name} {step}: blended_default={blended}")

    out = outdir / f"{engine_name}_{tag}.jsonl"
    with open(out, "w", encoding="utf-8") as fh:
        for rec in records:
            fh.write(json.dumps(rec, ensure_ascii=False, default=float) + "\n")
    print(f"{engine_name} {tag}: {len(records)} steps -> {out}")


def load_results(tag: str, outdir: str = "results") -> dict:
    """Read the consolidated JSONL files into {(step, engine): record}. Falls back
    to the 60k run when the requested tag is missing, so notebooks render even
    before the 1.5M run exists."""
    import json
    from pathlib import Path
    ...
    def read(t):
        out = {}
        for eng in ("main", "v05"):
            path = Path(outdir) / f"{eng}_{t}.jsonl"
            if path.exists():
                for line in path.read_text(encoding="utf-8").splitlines():
                    if line.strip():
                        rec = json.loads(line)
                        out[(rec["step"], eng)] = rec
        return out

    recs = read(tag)
    if not recs and tag != "60k":
        recs = read("60k")
    return recs
