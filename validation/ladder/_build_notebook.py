...
c.append(nbf.v4.new_code_cell(
    ...
    "    mk = sum(r['keep_in_vol'] for r in m['decile_table']); vk = sum(r['keep_in_vol'] for r in v['decile_table'])\n"
    "    ms = sum(r['swap_in_vol'] for r in m['decile_table']); vs = sum(r['swap_in_vol'] for r in v['decile_table'])\n"
    "    print('keep_in_vol          main', round(mk), 'v05', round(vk), '<-- fonte da divergência' if abs(mk-vk)>1 else '')\n"
