...
def main():
    ...
    print("=== MODEL KS ON df (Entire Population) ===")
    ev_all = ModelEvaluator(df, all_scores, "actual_default")
    ks_all = ev_all.compute_ks()
    for k, v in ks_all.items():
        print(f"{k}: {v:.4f} ({v*100:.2f}%)")

    print("\n=== MODEL KS ON df_hist (Hired Portfolio) ===")
    ev_hist = ModelEvaluator(df_hist, all_scores, "actual_default")
    ks_hist = ev_hist.compute_ks()
    for k, v in ks_hist.items():
        print(f"{k}: {v:.4f} ({v*100:.2f}%)")
