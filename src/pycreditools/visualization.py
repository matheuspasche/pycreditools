...
def plot_vintage_stability(
    ...
    if oot_start_safra:
        # Draw vertical line
        try:
            ax.axvline(x=oot_start_safra, color="red", linestyle="--", lw=2, label="Divisor DEV/OOT")
        except Exception:
            pass
        ...
        try:
            ...
            if oot_start_safra in idx_list:
                ...
                ax.text(dev_label_pos, y_text, "Desenvolvimento (DEV)", color="blue", fontsize=11, fontweight="bold", ha="center")
                ax.text(oot_label_pos, y_text, "Fora do Tempo (OOT)", color="red", fontsize=11, fontweight="bold", ha="center")
            else:
                pos = len(idx_list) // 2
                ax.text(idx_list[max(0, pos - 2)], y_text, "Desenvolvimento (DEV)", color="blue", fontsize=11, fontweight="bold", ha="center")
                ax.text(idx_list[min(len(idx_list) - 1, pos + 2)], y_text, "Fora do Tempo (OOT)", color="red", fontsize=11, fontweight="bold", ha="center")
        except Exception:
            pass
