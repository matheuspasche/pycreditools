...
def print_delta_table(
    ...
    if vol_new > 0:
        bad_new = (
            (df_new.loc[df_new["scenario"] == "keep_in", "simulated_default"] * df_new.loc[df_new["scenario"] == "keep_in", "new_approval"]).sum() +
            (df_new.loc[df_new["scenario"] == "swap_in", "simulated_default"] * df_new.loc[df_new["scenario"] == "swap_in", "new_approval"]).sum()
        ) / vol_new
    else:
        bad_new = 0.0
    ...
    if sim_old is not None:
        ...
        aprov_old = (df_old[old_approval_col] > 0).mean() if old_approval_col in df_old.columns else (df_old["new_approval"] > 0).mean()
        legacy_hired_col = "hired" if "hired" in df_old.columns else ("new_approval" if "new_approval" in df_old.columns else old_approval_col)
        ...
        aprov_old = (df_new[current_approval_col] > 0).mean() if current_approval_col in df_new.columns else 0.0
    ...
    print(f"{'Aprovação Global (% ToF)':<35} {aprov_old:>10.2%}  {aprov_new:>10.2%}  "
    ...
    print(f"{'Bad Rate Contratado (P&L)':<35} {bad_old:>10.2%}  {bad_new:>10.2%}  "
          f"{bad_diff_abs:>+10.2%}  {bad_diff_rel:>+10.1%}")
          
    # Volume comparison
    vol_diff_abs = vol_new - vol_old
    vol_diff_rel = (vol_new / vol_old) - 1.0 if vol_old > 0 else 0.0
    print(f"{'Vol. Contratado Esperado':<35} {vol_old:>10,.0f}  {vol_new:>10,.0f}  "
...
def print_quadrant_summary(sim_results: CreditSimResults) -> None:
    ...
    legacy_hired_col = "hired" if "hired" in df.columns else policy_dict.get("current_approval_col", "approved")
    so_vol = df.loc[df["scenario"] == "swap_out", legacy_hired_col].sum() if legacy_hired_col in df.columns else 0.0
    if so_vol > 0:
        so_bad = (df.loc[df["scenario"] == "swap_out", actual_default_col] * df.loc[df["scenario"] == "swap_out", legacy_hired_col]).sum() / so_vol
    else:
        so_bad = 0.0
    ...
    print(f"{'Swap In':<12} {si_vol:>17,.0f}  {si_bad:>12.2%}  simulated_default (agravado/simulado)")
    print(f"{'Swap Out':<12} {so_vol:>17,.0f}  {so_bad:>12.2%}  {actual_default_col} (observado na carteira antiga)")
...
def print_swap_in_by_rating(sim_results: CreditSimResults, rating_col: str = "Rating") -> None:
    ...
    si_r["Vol %"] = si_r["Vol_Esperado"] / si_r["Vol_Esperado"].sum() if si_r["Vol_Esperado"].sum() > 0 else 0.0
    ...
    for _, row in si_r.iterrows():
        print(f"{row[rating_col]:>8} {row['Vol_Esperado']:>15,.0f} {row['Vol %']:>8.1%} {row['Inad_Stressed']:>15.2%}")
...
def print_rating_quadrant_table(sim_results: CreditSimResults, rating_col: str = "Rating") -> None:
    ...
    def build_row(g):
        scen = g.name[1]
        if scen == "swap_out":
            aprov = int((g[current_approval_col] > 0).sum()) if current_approval_col in g.columns else 0
            ...
            if legacy_hired_col in g.columns and g[legacy_hired_col].sum() > 0:
                bad = (g[actual_default_col] * g[legacy_hired_col]).sum() / g[legacy_hired_col].sum()
            else:
                bad = np.nan
            return pd.Series({
                "Aprovados":    aprov,
