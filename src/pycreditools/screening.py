...
from ._kernels import ward_cluster, iv_cluster, calculate_tier_metrics
...
class ScreeningRecipe:
    ...
    def from_dict(cls, d: dict[str, Any]) -> ScreeningRecipe:
        return cls(
            ...
            sub_mappings={int(k): {int(sk): sv for sk, sv in v.items()} for k, v in d["sub_mappings"].items()}
        )
...
def screen_risk_segments(
    ...
    return ScreeningResult(
        ...
        recipes=recipes,
        params={"n_bins": n_bins, "method": method}
    )
