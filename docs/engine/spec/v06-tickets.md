...
**Estado do flake de `test_sweep.py:147` — medido neste ticket, e a caracterização não se sustentou.** Nesse teste o caminho estocástico **não sorteia nada**: toda linha de `risk_df` é aprovada e a política não tem estágio de taxa, então não há swap-in nem take-up a sortear. O stream global sai intocado da rodada, e o gap analítico × estocástico mede **0,0 nos quatro pontos da grade, em 100 sementes globais**. Não era flake latente; a tolerância absoluta de 0,05 não era banda de nada. **Pinado** como igualdade exata, com a premissa — nenhum sorteio global — asserida no teste. A mesma afirmação está em `v06-architecture.md:44` e `:1327`, que não se emendam sem ADR; a correção mora aqui. O irmão que sorteia de verdade, `test_method_does_not_move_the_default_rate`, fica em **quarentena declarada até o ticket 16**, com a tolerância reescrita em unidade de ruído: sd do gap medido em 0,00588 a n=4000, e os 0,03 absolutos eram 5,1 sd, com 0 de 200 sementes os alcançando.
**O que o ticket entrega, para quem o consome:** `tests/noise_discipline.py` (importável; `pythonpath = ["tests"]`) com `derive_seed`/`derive_seeds` e `--seed-offset`, `measure_sd` + `assert_within_k_sd(sd=, at_n=, n=, k=)`, `assert_unbiased(..., paired=, k=, terms=)` — pareamento declarado, sem default: `paired=False` é o método do #149 e o seguro sob a DoD 3; `paired=True` só quando a mão consome a primitiva do sorteio chaveado —, `assert_exact(..., terms=)`, `assert_reproducible`, e os dois guardas da §4.6: o de execução em `tests/engine/` e o tripwire de fonte sobre `src/pycreditools/engine/`.
