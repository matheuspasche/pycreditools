...
| população de calibração | **#139** fixou a população (o livro contratado) e registrou `"global"` como queimado. **#152**, na **resolução final** (2026-09-06 21:12, decisão 7, dono), escolheu **`calibrate_on="global"`** como **emenda declarada ao #139**. As camadas anteriores do mesmo card — a proposta (§3) e a sessão parada de 06/09 14:31 (decisão 4) — diziam `"hired"` | **Nada.** *Corrigido em 2026-09-10:* esta linha dizia que o #152 decidiu `"hired"`, lendo as camadas antigas do card. A spec §4.4 segue o card mais recente, e a §9.1 não precisava declarar nada — a decisão foi tomada **em card**. O **#182**, aberto sobre essa leitura, fechou por premissa falsa. Sobra a dívida que a própria §4.4 declara: a mesma palavra dá **bordas diferentes** em v0.5 e v0.6, então a paridade do ticket 15 não pode casar `calibration_base="global"` com `calibrate_on="global"` pelo nome |
